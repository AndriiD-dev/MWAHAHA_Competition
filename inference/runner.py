...
class InferenceRunner:
    ...
    def run(self) -> None:
        ...
        for batch_index, batch_ids in enumerate(batched(indices, self.cfg.batch_size)):
            ...
            if batch_index == 0 and batch_ids:
                ...
                logger.log_first_in_batch(
                    ...
                    example_index=first_i,
                    chat_text=batch_texts[0],
                    model_output=batch_out[0] if batch_out else "",
                    meta=metas[first_i] if 0 <= first_i < len(metas) else None,
                    note="first plan batch sample",
                )
        ...
        for retry_round in range(1, self.cfg.max_retries + 1):
            ...
            print(f"[RETRY] round={retry_round} bad={len(bad_indices)}")
            ...
            failed_items.append(
                {
                    ...
                    "id": row_id,
                    "mode": mode,
                    "humor_type": humor_types[i],
                    "anchors": (metas[i] or {}).get("anchors"),
                    "headline": (metas[i] or {}).get("headline"),
                    "plan": (metas[i] or {}).get("plan"),
                ...
                fallback_items.append(
                    {
                        ...
                        "headline": (metas[i] or {}).get("headline"),
                        "plan": (metas[i] or {}).get("plan"),
                        "last_prediction_before_fallback": last_before_fallback[i],
